    ...
    public abstract class Personnage : Microsoft.Xna.Framework.DrawableGameComponent, IPause
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (TempsÉcouléDepuisMAJ >= IntervalleMAJ)
            {
                ...
                VecteurVitesse += VecteurVitesseGaucheDroite;
        ...
        private void GérerFriction()
        {
            ...
            {
                if (VecteurVitesse.X < 0 && (VecteurVitesse - VecteurGauche * mu * Atelier.ACCÉLÉRATION_GRAVITATIONNELLE_PERSONNAGE).X < 0)
                    VecteurVitesse -= VecteurGauche * mu * Atelier.ACCÉLÉRATION_GRAVITATIONNELLE_PERSONNAGE;
                else if (VecteurVitesse.X > 0 && (VecteurVitesse + VecteurGauche * mu * Atelier.ACCÉLÉRATION_GRAVITATIONNELLE_PERSONNAGE).X > 0)
                    VecteurVitesse += VecteurGauche * mu * Atelier.ACCÉLÉRATION_GRAVITATIONNELLE_PERSONNAGE;
                else
        ...
        public void EncaisserDégâts(Personnage p)
        {
            if (!EstBouclierActif)
            {
                    ...
                    {
                        VecteurVitesse += ForceCoup *Vector3.Normalize(new Vector3(1,0.1f,0))*TempsÉcouléDepuisMAJ * (1+VieEnPourcentage / 100f) / Masse;
                    }
                    else
                    {
                        VecteurVitesse += ForceCoup * Vector3.Normalize(new Vector3(-1, 0.1f, 0)) * TempsÉcouléDepuisMAJ*(1+VieEnPourcentage/100f) / Masse;
        ...
        public void EncaisserDégâts(Projectile p)
        {
            if (!EstBouclierActif)
            {
                ...
                if (p.Direction == ORIENTATION.DROITE)
                {
                    VecteurVitesse += TempsÉcouléDepuisMAJ * ForceCoup * Vector3.Right / Masse * VieEnPourcentage/100;
                }
                else
                {
                    VecteurVitesse += TempsÉcouléDepuisMAJ * ForceCoup * Vector3.Left / Masse * VieEnPourcentage/100;
                }
            }
        }
        #endregion
